...
export class UpdateScheduleModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    const currentDate = new Date(this.schedule.trainDate);
  ...
  handleSaveForm() {
    ...
    if (this.timeNextSelect == this.schedule.trainDate) {
      return this.toast.error('Vui lòng chọn ngày muốn chuyển đến!');
    }
    let data = {
      id: this.schedule.id,
